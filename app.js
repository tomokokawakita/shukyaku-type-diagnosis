...
$('#share-button').onclick=async()=>{const text=`私の集客タイプは「${types[ranked[0]].name}」でした！\n${types[ranked[0]].catch}\n#あなたにぴったりの集客診断`;try{if(navigator.share)await navigator.share({title:'あなたにぴったりの集客診断',text});else{await navigator.clipboard.writeText(text);toast()}}catch(e){if(e.name!=='AbortError'){await navigator.clipboard.writeText(text);toast()}}};
function toast(){const t=$('#toast');t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2200)}
